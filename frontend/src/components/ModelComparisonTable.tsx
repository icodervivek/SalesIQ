      ...
      <table className="w-full text-sm">
        ...
        <tbody>
          {valid.map((r) => {
            ...
            return (
              <tr key={r.name} className={clsx("border-b border-surface-border/60 last:border-0", isBest && "bg-brand-600/[0.06]")}>
                <td className="py-3 pr-4">
                  <div className="flex items-center gap-2">
                    {isBest && <Trophy size={14} className="text-accent-amber" />}
                    <span className={clsx("font-medium", isBest ? "text-white" : "text-gray-300")}>{r.name}</span>
                  </div>
                </td>
                <td className="px-4 text-gray-300">{r.metrics.mae?.toFixed(2)}</td>
                <td className="px-4 text-gray-300">{r.metrics.rmse?.toFixed(2)}</td>
                <td className="px-4 text-gray-300">{r.metrics.mape?.toFixed(2)}%</td>
                <td className="pl-4 text-gray-300">{r.metrics.r2?.toFixed(3)}</td>
              </tr>
            );
          })}
        </tbody>
